...
object TianChiLineYc {
  ...
  def main(args:Array[String]): Unit = {
    ...
    if(true){
      sqlContext.sql("select * from dayall where ds<'20151031' and ds>'20150831' ").registerTempTable("dayn")
      sqlContext.sql("select * from tianchi_users where ds<'20150831' and ds>'20150630'").registerTempTable("usa")
      sqlContext.sql("select * from tianchi_users where ds<'20150831' and ds>'20150630' ").registerTempTable("temp")
      ...
      sqlContext.sql("select artistId,count(*) as num from songIntId where publishTime>'20150630' and publishTime<'20150831' group by artistId")
        .registerTempTable("ansn")
      //sqlContext.sql("select * from adn where ds<'20150701' and ds>'20150430'").registerTempTable("adnn")
      sqlContext.sql("select a.* ,b.ds from artists a join dayn b ").registerTempTable("adnn")
      sqlContext.sql("select * from adn where ds<'20150831' and ds>'20150630' ").registerTempTable("adnq")
    ...
    val yb=sqlContext.sql("select ds,artistId,weekday,isVa,gender,isNull(awb),isNull(aws),isNull(awx)," +
